...
class DiceLoss(nn.Module):
    ...
    def forward(self, input: Tensor, target: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        ...
        if self.reduction == "mean":
            print(f"++++++ TRAIN LOSS: {loss / logits_size}")
            # print(loss.mean())
            # return loss.mean()
            return loss / logits_size
    ...
    def _compute_dice_loss(self, flat_input, flat_target):
        ...
        # interection = torch.sum(flat_input * flat_target)
    ...
    def _multiple_class(self, input, target, logits_size, mask=None):
        flat_input = input # num batch x tokens x num labels
        flat_target = F.one_hot(target, num_classes=logits_size).float() if self.index_label_position else target.float() # num batch x tokens x num labels
        flat_input = torch.nn.Softmax(dim=1)(flat_input) if self.with_logits else flat_input # num batch x tokens x num labels
        ...
        if self.ohem_ratio > 0 :
            ...
            for label_idx in range(logits_size):
                """
                Compute the loss for each label in a batch. To do this, select all
                input (and target) values for that label across the batch using
                flat_input_idx (shape: batch size x number of tokens).
                These tensors are then flattened into a single dimension
                (e.g., flat_input_idx.contiguous().view(-1)) and the loss is computed.
                """
                pos_example = target == label_idx 
                """
                Select the scores for the given label (label_idx) for every batch
                element (each sentence). There will be as many values as tokens in
                the batch, each corresponding to the score of that label for that token.
                """
                flat_input_idx = flat_input[:, :, label_idx]    # batch size x num tokens
                flat_target_idx = flat_target[:, :, label_idx]
                
                # 1D tensor; number of elements equals batch size x number of tokens
                # loss_idx = self._compute_dice_loss(flat_input_idx.contiguous().view(-1, 1), flat_target_idx.contiguous().view(-1, 1))
                loss_idx_test = self._compute_dice_loss(flat_input_idx.contiguous().view(-1), flat_target_idx.contiguous().view(-1))
                # print(loss_idx)
                # print(loss_idx_test)
                if loss is None:
                    # loss = loss_idx
